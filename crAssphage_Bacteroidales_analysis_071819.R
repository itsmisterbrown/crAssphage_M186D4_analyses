...
View(cdf)
#add read count
cdf$readcount <- c(2058, 35, 5099, 8809, 55, 57, 108, 46)
cdf$nbases <- c(71931, 4476, 79569, 96906, 6837, 6359, 26684, 5774)

#define rpkm function
rpkm.dyn <- function(nreads, gsize, totreads){
  rpkm.c <- nreads/(gsize/1000*totreads/1000000)
  return(rpkm.c)
}
#add rpkm calcs
cdf$rpkm <- rpkm.dyn(nreads = cdf$readcount, gsize = cdf$nbases, totreads = rep(10000000, nrow(cdf)))
...
#GLM crassphage FC
lmt <- glm(formula = sigvec~log10(rpkm), data = cdf, family = gaussian(link = identity))
lmt <- glm(formula = sigvec~rpkm, data = cdf, family = gaussian(link = identity))
...
plot1 <- ggplot(po.bm.cor4, aes(x = OTU, y = Abundance, fill = FCbin)) +
  guides(ncol = 1, fill=guide_legend(title = "crAssphage fold coverage", nrow = 1)) +
  geom_boxplot(size=1.5) +
  theme(axis.text.y = element_text(size = 20, colour = "black"),
...
plot2 <- ggplot(cdf, aes(x=FC, y=sigvec)) +
  geom_point(size=5, color="steelblue4") +
  geom_point(color = "grey90", size = 2) +
  theme(axis.text.y = element_text(size = 20, colour = "black"),
  ...
  geom_smooth(method = "glm", data = cdf, formula = y~x, method.args = list(family = gaussian(link = identity)), se = TRUE) + 
  labs(y=expression(paste(log(italic(over("B.thetaiotaomicron","P.merdae"))))), x="crAssphage fold coverage") +
...
#constrained PCA
#create alias
phy.obj <- inf.vs.crass.clr
otu_table(phy.obj)
sample_data(phy.obj)

edist <- phyloseq::distance(physeq = phy.obj, method = "euclidean", weighted=F)
...
cap_plot <- plot_ordination(
  ...
  geom_point(aes(colour = FCbin), size = 4)
